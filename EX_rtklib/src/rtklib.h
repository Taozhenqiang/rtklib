...
/* constants -----------------------------------------------------------------*/
#define EXBDS_2 "C01 C02 C03 C04 C05 C06 C07 C08 C09 C10 C11 C12 C13 C14 C16 C59 C60 C61"
#define EXBDS_3 "C01 C02 C03 C04 C05 C19 C20 C21 C22 C23 C24 C25 C26 C27 C28 C29 C30 C32 C33 C34 C35 C36 C37 C38 C39 C40 C41 C42 C43 C44 C45 C46 C56 C57 C58 C59 C60 C61"
...
#define initlock(f) InitializeCriticalSection(f)//临界区锁
#define lock(f)     EnterCriticalSection(f)//加线程锁
#define unlock(f)   LeaveCriticalSection(f)//解除线程锁
#define FILEPATHSEP '\\'
#else
...
typedef struct {        /* time struct */
    time_t time;        /* time (s) expressed by standard time_t */ //从（1970年01月01日 0:00:00)开始计算的整秒数
...
typedef struct {        /* GPS/QZS/GAL broadcast ephemeris type */
	...
	double delA,Adot,delndot,acc;
	int SatType,DataFrom;
...
typedef struct {        /* navigation data type */
	...
	double ion_bds[24][8];  /* BeiDou iono model hourly parameters {a0,a1,a2,a3,b0,b1,b2,b3} */
...
typedef struct {        /* solution type */
	...
	double dop[4];      /* dilution of position */
	int satnum[5];      /* number of satellites of different system(GPS/GLO/Galileo/BDS2/BDS3) */
} sol_t;
...
typedef struct {        /* satellite status type */
	...
	double ion[NFREQ];  /* ionospheric correction of satellite(m) */
...
EXPORT double ionmodel(gtime_t t, const double *ion, const double *pos,
                       const double *azel);
EXPORT double ionmodel_BDS(gtime_t t, const double *ion, const double *pos,
                       const double *azel);
