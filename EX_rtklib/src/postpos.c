    ...
    while ((nobs=inputobs(obs,rtk.sol.stat,popt))>=0) {
        ...
        for (i=n=0;i<nobs;i++) {
			if ((satsys(obs[i].sat, NULL)&popt->navsys) &&
				...
				obs[n++] = obs[i];//剔除未参与解算系统卫星与手动去除的卫星
				//trace(1, "include_sat:rcv=%d id=%-3s sat=%3d\n",obs[i].rcv,id,obs[i].sat);  //测试
			}
        }
        if (n<=0) continue;
...
static void combres(FILE *fp, const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    solstatic=sopt->solstatic&&
              (popt->mode==PMODE_STATIC||popt->mode==PMODE_PPP_STATIC);//solstatic为静态解决方案模式，置0则输出全部解决方案
    ...
    for (i=0,j=isolb-1;i<isolf&&j>=0;i++,j--) {
        ...
        if ((tt=timediff(solf[i].time,solb[j].time))<-DTTOL) {
			...
			for (k=0;k<3;k++) rbs[k]=(rbf[k+i*3]+rbb[k+j*3])/2;//测试
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    if (ts.time==0||te.time==0) {
        ...
        if (i<j) {
			...
			trace(3, "rcv=%d ts=%s   ", obs->data[i].rcv,time_str(ts, 3));
			trace(3, "te=%s\n",time_str(te,3));//测试,一个括号内使用两个time_str会出错
            settspan(ts,te);
        }
    }
    return 1;
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    if (popt_.mode==PMODE_FIXED) {
        if (!antpos(&popt_,1,&obss,&navs,stas,fopt->stapos)) {//获取移动站天线相位中心位置
            freeobsnav(&obss,&navs);
            return 0;
        }
    }
    else if (PMODE_DGPS<=popt_.mode&&popt_.mode<=PMODE_STATIC) {
        if (!antpos(&popt_,2,&obss,&navs,stas,fopt->stapos)) {//获取基准站天线相位中心位置
            freeobsnav(&obss,&navs);
            return 0;
        }
    }
                   ...
                   const char *rov, const char *base)
{
    ...
    if (!openses(popt,sopt,fopt,&navs,&pcvss,&pcvsr)) return -1;//读取接收机、卫星天线参数及水准面数据
    ...
    if (ts.time!=0&&te.time!=0&&tu>=0.0) {
        ...
        for (i=0;;i++) { /* for each periods */
            ...
            if (timediff(tte,te)>0.0) tte=te;//处理设置时间不能超过观测时间的范围
