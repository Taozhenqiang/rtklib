                  ...
                  const double *rb, const solopt_t *opt)
{
    ...
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
	int timeu = opt->timeu < 0 ? 0 : (opt->timeu > 20 ? 20 : opt->timeu);//测试
	const char *s3[] = { "GPST","UTC ","JST " };//测试
    trace(3,"outenu  :\n");
    
    for (i=0;i<3;i++) rr[i]=sol->rr[i]-rb[i];
    ecef2pos(rb,pos);
    soltocov(sol,P);//赋值P
    covenu(pos,P,Q);//将P由ecef坐标系转换为enu坐标系
    ecef2enu(pos,rr,enu);//将基线向量由ecef坐标系转换到enu坐标系
	trace(4, "Q=\n"); tracemat(4, Q, 3, 3, 10, 4);//测试
    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s"
               "%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\r\n",
	...
	trace(4,"%s  %-*s%s%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s"
		"%s%6s%s%6s\n",
		COMMENTH, (opt->timef?16:8)+timeu+1,s3[opt->times],sep,
		...
		"ratio");//测试
	trace(4,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s"
		"%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
		s, sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->stat, sep, sol->ns, sep,
		SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
		sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);//测试
    return p-(char *)buff;
}
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt)
{
    ...
    if (PMODE_DGPS<=opt->mode&&opt->mode<=PMODE_FIXED) {
        p+=sprintf(p,"%s freqs     : %s(%d %d %d)\r\n",COMMENTH,s2[opt->nf-1],q[0],q[1],q[2]);
    }
