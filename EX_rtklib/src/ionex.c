...
static int dataindex(int i, int j, int k, const int *ndata)
{
    ...
    return i+ndata[0]*(j+ndata[1]*k);//列为纬度，行为经度
}
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"START OF TEC MAP")==label) {
            ...
            for (m=0;m<n;m++) {
                ...
                if ((index=dataindex(i,j,k,p->ndata))<0) continue;//列为纬度，行为经度
...
{
    const double fact=40.30E16/FREQ1/FREQ1; /* tecu->L1 iono (m) *///TEC是以10E16个电子为单位的量
